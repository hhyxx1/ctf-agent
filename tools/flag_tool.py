...
FLAG_PATTERNS = [
    ...
    r"HCKERT\d*\{[^}]+\}",
    r"intigriti\{[^}]+\}",
    # 兜底：任意 前缀{内容} 且内容 6-120 字符，字符集限 CTF flag 常见字符
    # （排除空格/分号/引号/花括号/冒号/逗号/括号/等号——CSS 片段、JS 碎片如
    #  let{v:e,emoji:t}、try{let a=...} 全被拦掉；真 flag 极少含这些字符）
    r"[A-Za-z0-9_]{2,}\{[A-Za-z0-9_!@#$%^&*.\-+/?\\~|]{6,120}\}",
]
